...
from src.transpilers.spp.s import start
...
#    res = start(identities(classes(classAccesses(addEndMethods(news(types(parseTree)))))))
#    print(SPrettyPrinter().transform(res))
#    return res

def transpile(parseTree):
    for delta in [addEndMethods, types, classes, news, classAccesses, imports, globalAssignements, identities, start]: 
        try: parseTree = delta(parseTree)
        except Exception as e: continue #import traceback; traceback.print_exc(); print(delta, e);print(("="*20+"\n")*5); continue
    return parseTree


deltas = [types, globalAssignements, imports, news, addEndMethods, classAccesses, classes, identities, start]
