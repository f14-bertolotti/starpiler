...
from src.transpilers import removeSppClasses, toString
...
for testname in tests:
    def make():
        def f(self):
            self.assertEqual(run(program_string=toString(removeSppClasses(lang.parse(tests[f.__name__[5:]]["program"])))), tests[f.__name__[5:]]["result"])
